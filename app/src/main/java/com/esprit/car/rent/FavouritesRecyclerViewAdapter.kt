...
class FavouritesRecyclerViewAdapter(
...
) : RecyclerView.Adapter<FavouritesRecyclerViewAdapter.FilesViewHolder>() {

    lateinit var context: Context

    interface OnItemActionsListener {
        fun onDeleteCar(carConstructor:String?,carModel:String?,position: Int)
    }
    ...
    inner class FilesViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(item: Car,position: Int) {
            ...
            deleteBtn.setOnClickListener { onItemActionsListener.onDeleteCar(item.constructor,item.model,position) }
            when(item.model){
